...
namespace WebApplication.IntegrationTests
{
    public class UsersControllerTests : IClassFixture<WebApplicationFactory<Startup>>
    {
        ...
        public async Task GivenUnpopulatedUser_WhenUpdatingUser_ThenReturnBadRequest()
        {
            // Arrange
            HttpClient client = _factory.CreateClient();
            var url = $"{client.BaseAddress}Users/Update";
            ...
            var problemDetails = await responseMessage.DeserializeContentAsync<StatusCodeProblemDetails>();
            problemDetails.Detail.Should()
                            .Be("'Id' must be greater than '0'.;'Given Names' must not be empty.;'Last Name' must not be empty.;Email address is not valid.;'Mobile Number' must not be empty.;Mobile Number is not valid.");
        }
        ...
        [Fact]
        public async Task GivenNameJohn_WhenGettingUsers_ThenReturnJohnSmith()
        {
            // Arrange
            HttpClient client = _factory.CreateClient();
            var url = $"{client.BaseAddress}Users/Find?GivenNames=John";
            IEnumerable<UserDto> expectedUsers = new List<UserDto>
            {
                new UserDto
                {
        ...
        [Fact]
        public async Task GivenNoName_WhenGettingUsers_ThenReturnBadRequest()
        {
            // Arrange
            HttpClient client = _factory.CreateClient();
        ...
        public async Task GivenValidUser_WhenCreatingNewUser_ThenReturnCreatedUser()
        {
            // Arrange
            HttpClient client = _factory.CreateClient();
            var url = $"{client.BaseAddress}Users/Add";
            var command = _fixture.Build<CreateUserCommand>()
                                .With(x => x.EmailAddress, "u@info.com")
                                .With(x => x.MobileNumber, "0416868")
                                .Create();
            ...
            responseMessage.Headers.Location.Should()
                           .Be(new Uri($"{client.BaseAddress}Users?id={createdUser.UserId}"));
        ...
        // TEST NAME - createUser
        // TEST DESCRIPTION - Create user should fail for missing / invalid properties
        [Theory]
        [InlineData("", "", "", "")]
        [InlineData("Jake", "", "", "")]
        [InlineData("Jake", "Brian", "", "")]
        [InlineData("Jake", "Brian", "jake.brian@example.com", "")]
        [InlineData("Jake", "Brian", "ake.brian@example.com", "6655")]
        [InlineData("Jake", "Brian", "ake.brian@example", "06655")]
        ...
        public async Task GivenUserNotInDb_WhenUpdatingUser_ThenReturnNotFound()
        {
            ...
            UpdateUserCommand command = _fixture.Build<UpdateUserCommand>()
                                                .With(x => x.Id, 200)
                                                .With(x=>x.EmailAddress,"u@gmail.com")
                                                .With(x=>x.MobileNumber,"07646747")
                                                .Create();
